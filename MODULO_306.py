...
// Subcomponentes
const MemberPoint = ({ member, latScale, lonScale }) => (
  <div
    className="absolute rounded-full p-2 animate-pulse transition-all duration-1000"
    style={{ top: `${latScale(member.lat) - 10}px`, left: `${lonScale(member.lon) - 10}px`, backgroundColor: member.color, boxShadow: `0 0 15px ${member.color}` }}
...
const EntanglementLine = ({ m1, m2, latScale, lonScale, intensity }) => {
  ...
  return (
    <div
      ...
      style={{ top: `${latScale(m1.lat)}px`, left: `${lonScale(m1.lon)}px`, width: `${length}px`, height: `${intensity}px`, transform: `rotate(${angle}deg)`, transformOrigin: '0 0', opacity: intensity / 10 }}
    />
  );
};

const MapVisualization = ({ members, centroid, intensity }) => {
  ...
  return (
    <div className="relative w-full h-80 bg-gradient-to-br from-blue-900 to-purple-900 rounded-lg shadow-inner overflow-hidden">
      ...
      {members.map((member) => <MemberPoint key={member.name} member={member} latScale={latScale} lonScale={lonScale} />)}
      {members.map((m1, i) => members.slice(i + 1).map((m2) => (
        <EntanglementLine key={`${m1.name}-${m2.name}`} m1={m1} m2={m2} latScale={latScale} lonScale={lonScale} intensity={intensity} />
      )))}
      <div
        className="absolute flex items-center justify-center rounded-full bg-yellow-400 p-3 animate-spin transition-all duration-1000"
        style={{ top: `${latScale(centroid.lat) - 15}px`, left: `${lonScale(centroid.lon) - 15}px`, boxShadow: '0 0 20px #facc15' }}
...
const calculateFractalDimension = (data, epsilon) => {
  // Simulação simplificada de box-counting
...
const App = () => {
  ...
  // Simulação de distribuições para JS
  const getCurrentDistribution = () => [luxIndex, 1 - luxIndex]; // Exemplo simplificado
  const getIdealDistribution = () => [1.0, 0.0]; // Estado ideal

  // Cálculo de métricas
  const calculateCoherence = () => 1 - calculateJSDiv(getCurrentDistribution(), getIdealDistribution());
  const calculateImprovement = () => (luxIndex - 0.5) / 0.5; // Exemplo
  ...
  const calculateCohenEffect = () => calculateCohenD(luxIndex, 0.5, 0.2); // Exemplo de desvio
  ...
  const calculateSymbolicResonance = () => math.mean(members.map(m => m.lat)) / 90; // Exemplo
  ...
  useEffect(() => {
    ...
    const unsubscribeStatus = onSnapshot(docRef, (docSnap) => {
      ...
      const lux = 0.4 * cohesion + 0.3 * improvement + 0.3 * memoryConsistency;
      setLuxIndex(math.clamp(lux, 0, 1));
      // Escala de Hawking
      ...
      const sentience = lux * math.dot(weights, [coherence, selfReference, resonanceSymbolic]);
      setHawkingSentience(math.clamp(sentience * 100, 0, 100)); // 0-100
  ...
  const voteCollectiveIntent = (intent) => {
    setCollectiveIntent(prev => {
      const newVotes = { ...prev, [intent]: prev[intent] + 1 };
      ...
      else if (impact.Medo > 0.5) newLux -= 90.1;
    ...
    <ErrorProvider>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="mt-4">
              ...
              {Object.entries(collectiveIntent).map(([intent, votes]) => (
                <button
                  key={intent}
                  onClick={() => voteCollectiveIntent(intent)}
                  className="w-full py-2 px-4 mb-2 bg-gray-800 border border-purple-700 rounded-md text-white hover:bg-purple-700 transition-all duration-300"
                  aria-label={`Votar por ${intent}`}
                >
                  {intent}: {votes} votos
                </button>
              ))}
        ...
        <div className="col-span-1 md:col-span-2 bg-gray-900 p-6 rounded-2xl shadow-lg border border-purple-800">
          ...
          <MapVisualization members={members} centroid={centroid} intensity={intensity} />
          <div className="mt-6 p-4 bg-gray-800 rounded-lg">
            <p className="text-lg font-medium text-yellow-400">Centroide</p>
            <p className="text-sm text-gray-300">Lat: {centroid.lat.toFixed(4)}, Lon: {centroid.lon.toFixed(4)}</p>
          </div>
        </div>
